...
(defn number-input [{:keys [id label readonly placeholder on-submit on-change required value] :or {required false extra-class "" readonly false}}]
  #jsx [:div {:class "mb-3"}
        (l/label label)
        [:input {:type "number"
...
(defn bool-input [{:keys [ident label placeholder on-submit on-change readonly required value] :or {required false extra-class "" readonly false}}]
  #jsx [:div {:class "mb-3"}
        (l/label label)
        [:input {:type "checkbox"
                 :value (value)
