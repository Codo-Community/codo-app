...
(defn add-category-remote [ctx data parent-id]
  (let [vars (utils/remove-ns data)
        vars (dissoc vars :id)
        vars {:i {:content vars}}
        vars (if-not (u/uuid? (:id vars)) (merge-with merge vars {:i {:id (:category/id data)
                                                                      :content (utils/drop-false (-> vars :i :content))}})
                     vars)
  ...
  #jsx [:div {:class "flex flex-col ml-3 gap-1"}
        [:span {:class "flex flex-inline gap-2"
         ...
         [Show {:when (and (:hovering? (local)) (not (:editing? (local))))}
          [cm/ui-category-meny {:& {:ident [:category/id (:category/id (id))]
                                    :parent (:parent props)}}]]]
